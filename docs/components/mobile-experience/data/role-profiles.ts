...
export const roleProfiles: RoleProfile[] = [
  ...
  },
  {
    id: 'admin',
    name: '行政人员',
    duty: '水务行政审批、督查督办、公文流转与统计查看。先在列表认清「这是什么单」，再进详情分块看依据，最后底部固定完成审批动作。',
    permissions: ['审批流程处理', '督查任务下发', '公文流转', '统计报表查看与导出'],
    dataScope: ['审批业务数据', '督查整改进度', '部门效能统计', '政策法规库'],
    philosophy: '列表认单 → 详情分块看依据 → 底部固定审批。主次在信息架构，不在一堆同权按钮。',
    rules: [
      '列表页：分类（审批/督查/公文）+ 核心字段（类型、标题、来源部门、时限、紧急度），3 秒内看懂是啥。',
      '点列表进详情：同类数据分块（申请摘要 / 附件依据 / 历史意见），禁止一屏平铺杂糅。',
      '底部固定审批区：通过为主，驳回/补件为次；驳回须原因；缺关键附件不可盲批。',
      '不做闸控与片区巡检地图主路径。',
    ],
    homeIa: '待办列表（分类+核心字段）→ 审批详情（分块）→ 底部通过/驳回/补件',
    platform: '小程序适合待办触达与轻审批；大批量导出可降级 Web/桌面。',
    checkpoints: [
      '列表不点进详情也能认出单据类型与关键信息',
      '详情分块清晰，依据在操作之前',
      '审批按钮固定在底部，主次分明',
    ],
    defaultSceneId: 'adm-todo-list',
    scenes: [
      {
        id: 'todo-list',
        title: '待办列表',
        sceneId: 'adm-todo-list',
        businessScope: '行政人员处理待我审批、督查、公文等待办，先在列表完成「认单」。',
        dataView: '分类标签、标题、来源部门、时限、紧急度、当前节点；数量角标。',
        uiNorm: '列表页先做分类与核心数据展示，让人一眼看懂「这是啥信息」；主操作是点进详情，不是堆一排同权按钮。',
        recommend: '顶部分类（全部/审批/督查/公文）+ 列表卡（类型·标题·部门·时限·紧急度）。',
        avoid: '无分类无关键字段的标题墙，或列表行上塞满通过/驳回/转发。',
      },
      {
        id: 'todo-detail',
        title: '审批详情',
        sceneId: 'adm-todo-detail',
        businessScope: '打开单条待办后阅读依据并完成通过/驳回/补件。',
        dataView: '申请摘要、附件/依据清单、历史审批意见、当前可执行动作。',
        uiNorm: '详情里同类数据分块展示；底部审批按钮固定：主按钮「通过」，次要「驳回/退回补件」。',
        recommend: '分块：申请摘要 → 附件依据 → 历史意见；底栏固定审批。',
        avoid: '全文平铺 + 顶部一排同权操作，或审批按钮滚出屏外。',
      },
      {
        id: 'todo-flow',
        title: '列表到详情',
        sceneId: 'adm-todo-flow',
        businessScope: '完整路径：列表认单 → 点进详情分块阅读 → 底部审批。',
        dataView: '列表核心字段 + 详情分块字段 + 审批结果反馈。',
        uiNorm: '一次交互演示完整主路径：列表分类可读 → 进入详情分块 → 底部固定通过/驳回。',
        recommend: '点主按钮进入详情，详情底栏完成审批，可返回列表。',
        avoid: '列表直接点「通过」无阅读，或详情无固定底栏。',
      },
    ],
  },
  {
