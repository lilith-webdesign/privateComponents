<template>
  <fullscreen v-model="isFullscreen" :teleport="true" :page-only="true">
    ...
    >
    <!-- 1. 3D WebGL 数字水资源流体背景层 (ThreeJS Background) -->
    <ThreeBackground :mouse-x="xOffset" :mouse-y="yOffset" />
    ...
    <!-- 2. 双栏布局结构 (Double-Column Layout) -->
    <!-- 左半栏：留白区域 (仅在非全屏模式下占位) -->
    <div v-if="!isFullscreen" class="w-[55%] h-full relative z-10"></div>

    <!-- 右半栏：Fluent Acrylic 登录容器 (在全屏模式下绝对定位浮动，去除双栏边框限制) -->
    <div 
      :class="[
        isFullscreen 
        ...
        >
          ...
          <div class="card-front w-full h-full bg-transparent p-6 flex flex-col justify-between select-none relative">
            ...
            <div class="space-y-1 text-center">
              <h2 class="text-base font-extrabold tracking-wide text-slate-900 dark:text-white">控制中心账户登录</h2>
              <p class="text-[9px] text-slate-500 font-medium">使用您的专网加密身份证书登录</p>
            </div>
  ...
  <svg style="position: absolute; width: 0; height: 0; pointer-events: none; overflow: hidden;" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <filter id="liquid-glass-filter">
        <!-- baseFrequency 设定低频 fractalNoise 以形成柔和宽广的水波波纹 -->
        <feTurbulence 
          type="fractalNoise" 
...
const handleParallax = (e) => {
  ...
  // 计算鼠标在容器中的归一化坐标值 (-1 到 1)
  const x = (e.clientX - rect.left) / rect.width * 2 - 1;
  const y = (e.clientY - rect.top) / rect.height * 2 - 1;
  
  // 限制偏移值，用作 ThreeJS 相机的运动参考
  xOffset.value = x * 30;
  yOffset.value = y * 30;
};
...
const handleLogin = () => {
  ...
  if (!loginForm.value.account || !loginForm.value.password) {
    isShaking.value = true;
    ElMessage.warning('请输入完整的水网控制中心账号与密码！');
    return;
  }

  ElMessage.success('身份验证成功！已连接三维数字孪生大屏。');
};
...
// 4. 全屏视口模式控制 (由第三方 vue-fullscreen 统一接管)
...
const animateFilter = () => {
  // 平缓递增 seed，使滤镜噪点形成非常缓慢的波动效果，模拟水流折射
...
.liquid-glass-fluid-layer {
  position: absolute;
  /* 适当向外扩张，避免 SVG 滤镜扭曲后在卡片边缘处产生虚影或缝隙 */
  inset: -16px;
  /* 渐变底色 */
  ...
  /* 应用置换波纹滤镜，让里面的反射光影产生水流般的运动，但不影响最外层实线边框 */
